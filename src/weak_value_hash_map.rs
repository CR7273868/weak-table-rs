...
mod serde_impl {
    ...
    // Implement Serialize for WeakValueHashMap
    impl<K, V, S> Serialize for WeakValueHashMap<K, V, S>
    where
        K: Serialize + Eq + Hash,
        V: WeakElement,
        V::Strong: Serialize,
        S: BuildHasher,
    {
        ...
        where
            SE: Serializer,
        {
            // First, collect all valid entries (filter out expired weak references)
            let mut valid_entries = Vec::new();
            for (key, value) in self {
                valid_entries.push((key, value));
            }

            // Now serialize as a map
